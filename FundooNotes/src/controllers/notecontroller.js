...
export const newNote = async (req, res, next) => {
  ...
  export const updateArchieve=async(req,res,next)=>{
    try {
      const data = await noteservice.isarchieve(req.params._id);
      res.status(HttpStatus.ACCEPTED).json({
        code: HttpStatus.ACCEPTED,
        data: data,
        message: 'Archived opertion Done'
      });
    }
     catch (error) {
      res.status(HttpStatus.BAD_REQUEST).json({
  ...
  export const updateTrash=async(req,res,next)=>{
    try {
      const data = await noteservice.istrash(req.params._id);
      res.status(HttpStatus.ACCEPTED).json({
        code: HttpStatus.ACCEPTED,
        data: data,
        message: 'trash opertion Done'
      });
    }
     catch (error) {
      res.status(HttpStatus.BAD_REQUEST).json({
  ...
  export const updateColor=async(req,res,next)=>{
    try {
      const data = await noteservice.upColor(req.params._id,req.body);
      res.status(HttpStatus.ACCEPTED).json({
        code: HttpStatus.ACCEPTED,
        data: data,
        message: 'color update   Done'
      });
    }
     catch (error) {
      res.status(HttpStatus.BAD_REQUEST).json({
